...
Because the key's length is completely unknown, it breaks classical **[Frequency Analysis](../concepts/frequency-analysis.md)**. The letter 'E' in the plaintext might be encrypted by the 1st byte of the key, but the next time 'E' appears, it might be encrypted by the 4th byte of the key! This destroys the statistical distribution, making the cipher look indistinguishable from randomness.
...
### The Magic Trick: Finding Key Length via [Hamming Distance](../concepts/hamming-distance.md)
...
5. **Single-Byte Attack**: You now have `KEYSIZE` number of columns. As visualised above, every byte in a single column was encrypted using just one character of the key. You can now effortlessly break each column individually using standard **[Frequency Analysis](../concepts/frequency-analysis.md)** (**[Single-Byte XOR Attack](single-byte-xor.md)**).
